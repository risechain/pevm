...
impl Pevm {
    ...
    pub fn execute_revm_parallel<S: Storage + Send + Sync, C: PevmChain + Send + Sync>(
        ...
        for memory_location in mv_memory.consume_lazy_locations() {
            let location_hash = self.hasher.hash_one(memory_location.clone());
            let Some(write_history) = mv_memory.data.get(&location_hash) else {
                continue;
            };
                ...
                MemoryLocation::Storage(address, index) => {
                    ...
                    for (tx_idx, memory_entry) in write_history.iter() {
                        match memory_entry {
                            ...
                            MemoryEntry::Data(_, MemoryValue::ERC20TransferRecipient(addition)) => {
                                storage_value += addition
                            }
                            MemoryEntry::Data(_, MemoryValue::ERC20TransferSender(subtraction)) => {
